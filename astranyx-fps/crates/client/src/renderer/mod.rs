...
pub mod meshes {
    ...
    /// Create a simple arm mesh for first-person view.
    /// Arms positioned in bottom-right corner of screen.
    pub fn create_fps_arm_mesh() -> MeshData {
        let mut builder = MeshBuilder::new();
        // Forearm - elongated box
        builder.add_box(0.15, 0.1, 0.6);
        builder.finish()
    }

    /// Create a simple handgun mesh for first-person view.
    pub fn create_fps_weapon_mesh() -> MeshData {
        let mut builder = MeshBuilder::new();
        // Gun body - box
        builder.add_box(0.08, 0.12, 0.35);
        builder.finish()
    }

    /// Create a muzzle flash mesh (flat plane).
    pub fn create_muzzle_flash_mesh() -> MeshData {
        let mut builder = MeshBuilder::new();
        // Simple flat diamond for muzzle flash
        builder.add_diamond(0.15);
        builder.finish()
    }

    /// Create an FPS guard/soldier enemy mesh.
    /// A simple humanoid shape made of boxes.
    pub fn create_fps_guard_mesh() -> MeshData {
        let mut builder = MeshBuilder::new();
        // Body (torso) - main box
        builder.add_box(0.6, 1.0, 0.3);
        builder.finish()
    }

    /// Create an FPS guard head mesh.
    pub fn create_fps_guard_head_mesh() -> MeshData {
        let mut builder = MeshBuilder::new();
        builder.add_box(0.35, 0.4, 0.35);
        builder.finish()
    }

    /// Create an FPS sniper enemy mesh (taller, thinner).
    pub fn create_fps_sniper_mesh() -> MeshData {
        let mut builder = MeshBuilder::new();
        builder.add_box(0.5, 1.2, 0.25);
        builder.finish()
    }

    /// Create an FPS heavy enemy mesh (bulkier).
    pub fn create_fps_heavy_mesh() -> MeshData {
        let mut builder = MeshBuilder::new();
        builder.add_box(0.8, 1.0, 0.5);
        builder.finish()
    }
}

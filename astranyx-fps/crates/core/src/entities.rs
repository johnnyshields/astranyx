...
pub struct Player {
    ...
    // FPS physics state flags (Quake-style pmove)
    pub pm_flags: u16,     // PmFlags bitfield
    pub pm_time: u16,      // Time counter for jump cooldown, etc.
...
impl Player {
    ...
    /// Check if player is on the ground (FPS physics).
    pub fn is_on_ground(&self) -> bool {
        self.pm_flags & 0x01 != 0 // PmFlags::ON_GROUND
    }

    /// Check if player is crouching (FPS physics).
    pub fn is_ducking(&self) -> bool {
        self.pm_flags & 0x04 != 0 // PmFlags::DUCKING
    }
    ...
    pub fn eye_position(&self) -> Vec3 {
        let eye_height = if self.is_ducking() { 0.8 } else { 1.6 };
        self.position_3d + Vec3::new(0.0, eye_height, 0.0)
    }

    /// Get player height based on current posture.
    pub fn height(&self) -> f32 {
        if self.is_ducking() { 1.0 } else { 1.8 }
    }
}

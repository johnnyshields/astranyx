...
//! - Quake-style player movement physics (pmove)
...
/// Update all players in FPS mode using Quake-style pmove physics.
pub fn update_players(sim: &mut Simulation, inputs: &[PlayerInput]) {
    ...
    let collision_backend = collision::CollisionBackend::Legacy(&geometry);
    ...
    for (i, player) in sim.state.players.iter_mut().enumerate() {
        ...
        match &mode {
            GameMode::FirstPerson => {
                // Convert PlayerInput to pmove UserCmd
                let cmd = input_to_usercmd(&input, player);

                // Create pmove state from player
                let mut pm_state = player_to_pmove_state(player);

                // Run Quake-style physics
                pmove::pmove(&mut pm_state, &cmd, &collision_backend, Simulation::DT);

                // Copy state back to player
                pmove_state_to_player(&pm_state, player);

                // Generate footstep sounds if moving on ground
                if pm_state.pm_flags.is_on_ground() && pm_state.velocity.length_squared() > 1.0 {
                    let is_running = cmd.wants_sprint();
                    let is_crouching = cmd.wants_crouch();

                    // Footstep every ~20 frames when walking
                    let interval = if is_crouching { 30 } else if is_running { 10 } else { 20 };
                    if sim.state.frame % interval == 0 {
                        sound_events.push(SoundEvent::footstep(
                            player.position_3d,
                            player.id,
                            is_running,
                            is_crouching,
...
/// Convert PlayerInput to pmove UserCmd.
fn input_to_usercmd(input: &crate::input::PlayerInput, _player: &Player) -> pmove::UserCmd {
    let mut cmd = pmove::UserCmd::default();
    ...
    // Mouse look (convert from quantized i16 to radians)
    let (dx, dy) = input.mouse_delta_radians(0.003);
    cmd.delta_angles = Vec3::new(dy, dx, 0.0); // pitch, yaw, roll

    // Buttons
    if input.fire() {
        cmd.buttons |= pmove::UserCmd::BTN_ATTACK;
    }
    if input.up() {
        cmd.buttons |= pmove::UserCmd::BTN_JUMP;
    }
    if input.focus() {
        cmd.buttons |= pmove::UserCmd::BTN_CROUCH;
    }
    if input.special() {
        cmd.buttons |= pmove::UserCmd::BTN_SPRINT;
    }

    cmd
}

/// Convert Player entity to PlayerMoveState.
fn player_to_pmove_state(player: &Player) -> pmove::PlayerMoveState {
    pmove::PlayerMoveState {
        origin: player.position_3d,
        velocity: player.velocity_3d,
        view_angles: Vec3::new(player.look_pitch, player.look_yaw, 0.0),
        cmd_angles: Vec3::ZERO,
        pm_flags: pmove::PmFlags(player.pm_flags),
        ground_entity: player.ground_entity,
        pm_time: player.pm_time,
    }
}

/// Copy PlayerMoveState back to Player entity.
fn pmove_state_to_player(state: &pmove::PlayerMoveState, player: &mut Player) {
    player.position_3d = state.origin;
    player.velocity_3d = state.velocity;
    player.look_pitch = state.view_angles.x;
    player.look_yaw = state.view_angles.y;
    player.pm_flags = state.pm_flags.0;
    player.ground_entity = state.ground_entity;
    player.pm_time = state.pm_time;

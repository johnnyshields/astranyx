...
//!
//! FPS-only simulation for isolated development.
...
impl Simulation {
    ...
    pub fn load_embedded_scripts(&mut self) {
        ...
        // Enemy scripts (FPS-relevant enemies only)
        ...
        // FPS World - starts directly at FPS segment
    ...
    pub fn tick(&mut self, inputs: &[PlayerInput]) {
        ...
        // FPS-only simulation - no scroll needed
        // Process player inputs (FPS mode)
    ...
    fn on_segment_enter(&mut self) {
        ...
        if let Some(config) = self.scripts.get_segment_config(segment_id) {
            ...
            let player_start = config.player_start
                .unwrap_or_else(|| glam::Vec3::new(0.0, 5.0, 0.0));

            // Reset each player to the start position (FPS mode)
            for (i, player) in self.state.players.iter_mut().enumerate() {
                ...
                player.look_yaw = std::f32::consts::PI;
...
mod tests {
    ...
    fn fps_segment_loads() {
        ...
        // Verify we start on FPS segment (4_base)
        assert_eq!(sim.state.level.segment_id, "4_base");
        assert!(matches!(sim.state.level.mode, GameMode::FirstPerson));

        // Run some simulation frames
        let inputs = vec![PlayerInput::from_bits(PlayerInput::FORWARD)];
        for _ in 0..100 {
            sim.tick(&inputs);
        }

        // Should still be on the same segment (no transitions in isolated mode)
        assert_eq!(sim.state.level.segment_id, "4_base");
    }

    #[test]
    fn fps_mode_detection() {
        use crate::level::GameMode;
        ...
        // Test turret mode detection (also FPS-like)
